...
- `examples/reviewer-contract-sample-packs/` for copy-ready reviewer-contract conformance packs with expected structured-result fixtures
...
Use `docs/single-reviewer-vs-consensus-classification.md` when you need a public-safe proposal for deciding when one calibrated reviewer is enough and when consensus or tie-break review is worth the extra local cost.
...
batch-review-compare ./reviews/previous-batch-summary.json ./reviews/current-batch-summary.json --matrix-output ./reviews/contradiction-coverage-matrix.md --matrix-json-output ./reviews/contradiction-coverage-matrix.json
...
- `--matrix-output` and `--matrix-json-output` write the rendered Markdown matrix and raw JSON matrix artifacts directly from the same comparison. The checked-in `examples/synthetic-multi-review-contradiction-coverage-matrix.expected.md` and `.expected.json` files demonstrate both artifact shapes from the public-safe fixture pair.
